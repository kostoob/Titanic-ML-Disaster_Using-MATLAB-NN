...
J = (1/(m)) * sum(sum(-1 * y .* log(a4)-(1-y) .* log(1-a4)));	

regularator = (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)) + sum(sum(Theta3(:,2:end).^2))) * (lambda/(2*m));
J=J+regularator; 
